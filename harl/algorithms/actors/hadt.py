...
class HADT(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, device=torch.device("cpu")):
        ...
        """
        super(HADT, self).__init__(args, obs_space, act_space, device)
        ...
        self.use_adv_scale = args["use_adv_scale"]
        if self.use_adv_scale:
            self.adv_scale = torch.nn.Parameter(torch.randn(1, dtype=torch.float32,requires_grad=True,device=self.device))
            torch.nn.init.ones_(self.adv_scale)
            self.actor_optimizer.add_param_group({"params": self.adv_scale})
    ...
    def update(self, sample):
        ...
        if self.use_adv_scale:
            surr1 = imp_weights * adv_targ * self.adv_scale
            surr2 = (
                torch.clamp(imp_weights, 1.0 - self.clip_param, 1.0 + self.clip_param)
                * adv_targ * self.adv_scale
            )

            if self.use_policy_active_masks:
                policy_action_loss = (
                    -torch.sum(factor_batch * torch.min(surr1, surr2), dim=-1, keepdim=True) # * self.adv_scale
                    * active_masks_batch
                ).sum() / active_masks_batch.sum()
            else:
                policy_action_loss = -torch.sum( #self.adv_scale *
