...
class GlobalReward(ABC, Resetable):
    ...
    # type of DataStore managed by the GlobalReward
...
class GlobalImgSARReward(ABC, Resetable):
    """Base class for simulation-wide data management."""
    ...
    def reward(self, new_data_dict: dict[str, Data], actions) -> dict[str, float]:
        """Call :class:`calculate_reward` and log cumulative reward."""
        reward = self.calculate_reward(new_data_dict, actions)
        for satellite_id, sat_reward in reward.items():
            self.cum_reward[satellite_id] += sat_reward
...
__all__ = ["GlobalReward", "DataStore", "Data", "GlobalImgSARReward"]
