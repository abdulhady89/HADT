...
class GeneralSatelliteTasking(Env, Generic[SatObs, SatAct]):
    def __init__(
        ...
        if sat_arg_randomizer is None:
            def sat_arg_randomizer(sats): return {}
        ...
        self.world_args_generator = self.world_type.default_world_args(
            **world_args)
    ...
    def _minimum_world_model(self) -> type[WorldModel]:
        ...
        types = set(
            sum(
                [satellite.dyn_type._requires_world()
                 for satellite in self.satellites],
                [],
            )
        )
    ...
    def _configure_logging(self, log_level, log_dir=None):
        ...
        ch.addFilter(logging_config.ContextFilter(
            env=self, proc_id=os.getpid()))
        ...
        if log_dir is not None:
            ...
            fh.addFilter(logging_config.ContextFilter(
                env=self, proc_id=os.getpid()))
    ...
    def reset(
        ...
        # self.simulator.ShowExecutionOrder()
        ...
        # module_data = self.simulator.ShowModuleData()
        # module_data.to_csv('module_list_new.csv',index=True)
    ...
    def _step(self, actions: MultiSatAct) -> None:
        ...
        if len(actions) != len(self.satellites):
            raise ValueError(
                "There must be the same number of actions and satellites")
        ...
        # should these be recorded in simulator
        previous_time = self.simulator.sim_time
        self.simulator.run()
        ...
        for satellite in self.satellites:
            if satellite.requires_retasking:
                satellite.logger.info(
                    f"Satellite {satellite.name} requires retasking")
...
class ConstellationTasking(
    ...
    def _get_info(self) -> dict[AgentID, dict]:
        ...
        common = {k: v for k, v in info.items(
        ) if k not in self.possible_agents}
...
class GeneralSatelliteTaskingCloud(Env, Generic[SatObs, SatAct]):
    def __init__(
        self,
        satellites: Union[Satellite, list[Satellite]],
        scenario: Optional[Scenario] = None,
    ...
    def _step(self, actions: MultiSatAct) -> None:
        ...
        self.reward_dict = self.rewarder.reward(new_data, actions)
