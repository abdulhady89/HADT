            ...
            if hasattr(satellite, "add_location_for_access_checking"):
                for target in self.targets:
                    satellite.add_location_for_access_checking(
                        object=target,
                        r_LP_P=target.r_LP_P,
                        # was satellite.sat_args_generator["imageTargetMinimumElevation"]
                        min_elev=np.radians(83),
                        type="target",
                    )
    ...
    def regenerate_targets(self) -> None:
        ...
        for oceanName, priority_level, cloud_cover, bearing in zip(ocean_list, tgtPriority, tgtCloud_cover, bearing_deg):
            ...
            ocean_grids = self.generate_grid_coordinates(
                ocean['lat'], ocean['lng'], spacing=self.location_offset, rows=3, cols=15, bearing_deg=bearing)  # bearing_deg=-6
...
class UserDefOceanTargetswithCloud(Scenario):
    ...
    def add_new_targets(self) -> None:
        ...
        input_csv_path = Path(os.path.realpath(
            __file__)).parent.parent / "_dat" / "ocean" / "2nd_WP4_input.csv"
        ...
        # self.visualize_target(self.targets,wcolor="yellow")
        for target in self.targets:
            self.visualize_target(target, wcolor="yellow")

    @vizard.visualize
    def visualize_target(self, target, vizSupport=None, vizInstance=None, wcolor=None):
        """Visualize target in Vizard."""
        if wcolor == None:
            color = vizSupport.toRGBA255("white")
        else:
            color = vizSupport.toRGBA255(wcolor)
...
class RandomOrbitalTargetswithCloud(Scenario):
    ...
    def regenerate_targets(self) -> None:
        ...
        for sat in self.satellites:
            ...
            tgt_sat = self.generate_regions_under_track(
                latitudes, longitudes, num_targets=self._n_targets)
    ...
    def generate_LEO_ground_track(self, inclination_deg=50.0, altitude_km=500,
        ...
        time_steps = np.linspace(0, total_time, int(
            num_points_per_orbit * num_orbits))
        ...
        for t in time_steps:
            ...
            true_anomaly = (mean_motion * t +
                            np.radians(phase_offset_deg)) % (2 * np.pi)

            # Latitude from inclination
            lat = np.degrees(
                np.arcsin(np.sin(np.radians(inclination_deg)) * np.sin(true_anomaly)))
    ...
    def generate_regions_under_track(self, latitudes, longitudes, num_targets=5, lat_offset_deg=0.05, lon_offset_deg=0.1):
        ...
        for i in range(0, len(latitudes), step):
            lat = latitudes[i] + \
                random.uniform(-lat_offset_deg, lat_offset_deg)
            lon = longitudes[i] + \
                random.uniform(-lon_offset_deg, lon_offset_deg)
