...
class BasicDynamicsModel(DynamicsModel):
    ...
    def setup_density_model(self) -> None:
        ...
        self.world.densityModel.addSpacecraftToModel(
    ...
    def setup_thruster_dyn_effector(self, priority: int = 996) -> None:
        ...
        self.thrFactory.addToSpacecraft(
            thrModelTag, self.thrusterSet, self.scObject)
        self.simulator.AddModelToTask(
    ...
    def setup_thruster_power(
        ...
        self.powerMonitor.addPowerNodeToModel(
            self.thrusterPowerSink.nodePowerOutMsg)

    def setup_eclipse_object(self) -> None:
        """Add the spacecraft to the eclipse module."""
        self.world.eclipseObject.addSpacecraftToModel(
            self.scObject.scStateOutMsg)
    ...
    def setup_power_sink(
        ...
        self.powerMonitor.addPowerNodeToModel(
            self.basePowerSink.nodePowerOutMsg)
    ...
    def setup_reaction_wheel_power(
        ...
        **kwargs,
        ...
        for i_device in range(self.rwFactory.getNumOfDevices()):
            ...
            powerRW.rwStateInMsg.subscribeTo(
                self.rwStateEffector.rwOutMsgs[i_device])
...
class LOSCommDynModel(BasicDynamicsModel):
    ...
    def setup_los_comms(
        ...
        self.losComms.primaryScStateInMsg.subscribeTo(
            self.scObject.scStateOutMsg)
        ...
        for sat_dyn in self.simulator.dynamics_list.values():
            if sat_dyn != self and sat_dyn.satellite.name not in self.los_comms_ids:
                self.losComms.addSpacecraftToModel(
                    sat_dyn.scObject.scStateOutMsg)
                self.los_comms_ids.append(sat_dyn.satellite.name)
                sat_dyn.losComms.addSpacecraftToModel(
                    self.scObject.scStateOutMsg)
...
class ImagingDynModel(BasicDynamicsModel):
    ...
    def setup_transmitter(
        ...
        **kwargs,
        ...
        if transmitterBaudRate > 0:
            self.logger.warning(
                "transmitterBaudRate should probably be negative.")
    ...
    def setup_instrument_power_sink(
        ...
        self.powerMonitor.addPowerNodeToModel(
            self.instrumentPowerSink.nodePowerOutMsg)
    ...
    def setup_transmitter_power_sink(
        ...
        self.powerMonitor.addPowerNodeToModel(
            self.transmitterPowerSink.nodePowerOutMsg)
...
class ManyGroundStationDynModel(ImagingDynModel):
    """Model that connects satellite to world ground stations."""
    ...
    @classmethod
    def _requires_world(cls) -> list[type["WorldModel"]]:
        return super()._requires_world() + [world.ManyGroundStationWorldModel]
...
class ManyGroundStationFullFeaturedDynModel(ManyGroundStationDynModel, LOSCommDynModel):
    """Convenience class for a satellite with ground station and line-of-sight comms."""
