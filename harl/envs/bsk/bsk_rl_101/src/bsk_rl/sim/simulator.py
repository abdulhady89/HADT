...
class Simulator(SimulationBaseClass.SimBaseClass):
    ...
    def __init__(
        ...
        for satellite in self.satellites:
            ...
            self.dynamics_list[satellite.name] = satellite.set_dynamics(
                self.sim_rate)
    ...
    def setup_vizard(self, vizard_rate=None, vizSupport=None, **vizard_settings):
        ...
        viz_proc.addTask(self.CreateNewTask(
            viz_task_name, mc.sec2nano(vizard_rate)))
        ...
        for sat in self.satellites:
            sat_batterry = sat.dynamics.powerMonitor.batPowerOutMsg
            sat_storage = sat.dynamics.storageUnit.storageUnitDataOutMsg

            # Create battery panel
            batteryPanel = vizInterface.GenericStorage()
            batteryPanel.label = f"Battery_{sat.name}"
            batteryPanel.units = "W.s"
            ...
            batteryInMsg.subscribeTo(sat_batterry)
            ...
            dataMonitor.label = f"Data_{sat.name}"
