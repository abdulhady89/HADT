...
def make_BSK_SingleSat_env(env_args, optim_challenge, randomness=None):
    # Specify the Satelllite Name:
    satellite_names = "OPT-SAT"

    # Common orbital parameters for all satellites
    inclination = 40.0          # degrees, fixed for all satellites
    ...
    # Define the satellite specifications as arguments:
    sat_args = dict(
        # Attitude Control:
        u_max=0.4,
        omega_max=0.1,
    ...
    sat = ImagingSatellite(satellite_names, sat_args)
    duration = env_args.orbit_num * 5700.0  # Simulation time for any number of orbits
    ...
    env = GeneralSatelliteTasking(
        ...
        # activate these lines to render visualization
