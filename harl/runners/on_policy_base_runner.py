...
class OnPolicyBaseRunner:
    ...
    def collect(self, step):
        ...
        if self.algo_args['model']['use_hidden_state']:
            rnn_states = np.array(rnn_state_collector).transpose(1, 0, 2)
        else:
    ...
    def restore(self):
        ...
        if not self.algo_args["render"]["use_render"] and not self.algo_args["model"]["use_transf_policy"]:
